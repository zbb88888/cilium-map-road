...
| **轴2 特性域** | 处理「什么业务」 | 身份 / 策略 / 服务LB / CT-NAT / 加密egress | 5 策略面 / 6 CT-NAT / 7 服务LB / 8 加密egress（+身份主链） |
| **轴3 横切** | 贯穿所有阶段/域 | 生命周期 / 装配 / 健康 / 状态存储 / CNI接口 | 9 生命周期 / 10 装配（+候选） |
...
| 特性域 | 控制 | 缓存 | 转发 | 观察 |
| --- | --- | --- | --- | --- |
| **身份**（主链） | `CachingIdentityAllocator`、`GlobalIdentity`、`labelsfilter`、`Endpoint` | `IPCache`、`EndpointManager`、`IDManager`、`Identity` | `BPFListener`、`VniKey`、`cilium_ipcache_vni`、`bpf_lxc` | `EndpointResolver`、`Flow.vni_id` |
...
| 健康/就绪 | `health`、`healthz`、`status` | 现折叠在 4 切面 |
| 状态存储 | kvstore、stateDB、CRD | 现散在 2/9 |
| CNI 接口 | CNI chaining、endpoint 创建 | 现折叠在 1 控制 |

## 5. 候选「第 11+ 面」在三轴中的归宿

| 候选面 | 三轴归宿 |
| --- | --- |
| 身份/认证面 | 轴2 新增「身份/认证」特性域（含 auth/mTLS/Spire） |
| 节点/拓扑面 | 轴2 新增「节点/拓扑」特性域 |
| IPAM/地址管理面 | 轴2 新增「IPAM」特性域（或并入身份主链） |
| 健康/就绪面 | 轴3 新增横切 |
| 状态存储面 | 轴3 新增横切 |
| 运行时/CNI 接口面 | 轴3 新增横切 |
